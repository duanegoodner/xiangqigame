...
def minimax_red(game_board: GameBoard, search_depth: int, alpha: int, beta: int, cur_player: PieceColor):
    global node_counter
    node_counter += 1
    cur_moves = game_board.calc_final_moves_of(color=cur_player)
    if cur_moves == set():
        if cur_player == PieceColor.RED:
            return BestMoves(best_eval=-100000, best_moves=set())
        else:
            return BestMoves(best_eval=100000, best_moves=set())
...
game_board = GameBoard(game_config['board_data'])

cProfile.run('minimax_red(game_board=game_board, search_depth=2, alpha=-100000,'
             ' beta=100000, cur_player=PieceColor.RED)')

# result = minimax_red(game_board=game_board, search_depth=2, alpha=-100000,
#                      beta=100000, cur_player=PieceColor.RED)
