...
class GameBoard:
    ...
    def cannon_moves(self, from_position: BoardSpace, color: int):
        ...
        for direction in search_directions:
            ...
            for empty_space in search_results.empty_spaces:
                cannon_moves.append(Move(from_position, empty_space))
    ...
    def chariot_moves(self, from_position: BoardSpace, color: int):
        ...
        for direction in search_directions:
            ...
            for empty_space in search_results.empty_spaces:
                chariot_moves.append(Move(from_position, empty_space))
            ...
            if bu.is_on_board(first_step) and not self.is_occupied(first_step):
                ...
                for second_step in second_steps:
                    if self.exists_and_passes_color_test(second_step, color):
                        horse_moves.append(Move(from_position, second_step))
    ...
    def standard_general_moves(self, from_position: BoardSpace, color: int):
        ...
        unit_vects = (bu.fwd_unit_vect(color), bu.rev_unit_vect(color)
                      ) + bu.sideways_unit_vect
        adjacent_spaces = (from_position.add_board_vector(vect) for vect in
                           unit_vects)

        for space in adjacent_spaces:
            if bu.is_in_castle_of(color, space) and self.get_color(space) != color:
                standard_moves.append(Move(from_position, space))
    ...
    def calc_temp_moves_from(self, position: BoardSpace) -> List[Move]:
        color = self.get_color(position)
        return self.piece_moves[self.get_type(position)](self, position, color)

    def calc_final_moves_from(self, position: BoardSpace) -> List[Move]:
        final_moves = []
