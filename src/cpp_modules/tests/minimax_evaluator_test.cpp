...
protected:
  ...
  GamePointsArrayBuilder piece_points_builder_ = GamePointsArrayBuilder(points_spec_path);
  GameBoard<HashCalculator> game_board_;
};

TEST_F(PiecePointsEvaluatorTest, EvaluateMove) {
  auto game_position_points = piece_points_builder_.BuildGamePointsArray();
