import abc
import numpy as np
from cpp_modules.src.pybind_modules.GameBoardPy import BoardSpace, GameBoard, Move, opponent_of, PieceColor, PieceType
...
class PiecePoints(MinimaxEvaluator):
    ...
    def evaluate_leaf(
        ...
        opponent_pts = self.get_player_total(opponent_of(cur_player),
                                             game_board)
    ...
    def rate_move(
        ...
        if game_board.GetColor(move.end) == opponent_of(cur_player):
            ...
            opponent_position_array = self._position_pts.vals[
                opponent_of(cur_player)][captured_piece_type]
            capture_val = (
