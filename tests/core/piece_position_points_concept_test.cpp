...
TEST_F(PiecePositionPointsBuilderTest, BuilderReturnsValidSharedPtr) {
  auto piece_position_points = builder_.build();
  ASSERT_NE(piece_position_points, nullptr) << "Builder should not return nullptr";

  ASSERT_EQ(piece_position_points.use_count(), 1) << "Initial use count should be 1";
}
