...
def test_red_get_search_summaries(starting_board):
    specified_starting_search_depth = 3

    move_evaluator = MinimaxMoveEvaluator64(
        evaluating_player=PieceColor.kRed,
        starting_search_depth=specified_starting_search_depth,
    ...
    assert search_time > datetime.timedelta(0.0)

    zobrist_key_size = move_evaluator.zobrist_key_size_bits()
    assert zobrist_key_size == 64

    move_evaluator_starting_search_depth = (
        move_evaluator.starting_search_depth()
    )
    assert (
        move_evaluator_starting_search_depth == specified_starting_search_depth
    )
